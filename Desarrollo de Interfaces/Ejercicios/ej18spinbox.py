from PyQt6.QtWidgets import QApplication, QMainWindow, QSpinBox# Se importa solo lo que necesitas
...
class MainWindow(QMainWindow):
    def __init__(self): # Esto es una función a la que le pasas "self" que es la propia clase ¿constructor?
        ...
        spinBox.setSingleStep(2) # Para ir de 2 en 2

        spinBox.valueChanged.connect(self.cambio) # Valor con sufijo

        spinBox.textChanged.connect(self.cambio) # Solo valor

        self.setCentralWidget(spinBox)

        # El double spinbox es para decimales
